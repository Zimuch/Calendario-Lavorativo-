# Calendario Lavorativo --------> https://zimuch.github.io/Calendario-Lavorativo/
Italiano 
## Introduzione
![Italiano](https://img.icons8.com/color/48/italy-circular.png) 

**Calendario Lavorativo** è un progetto nato per semplificare la gestione dei turni e automatizzare il calcolo dello stipendio, offrendo un'alternativa moderna e reattiva ai classici fogli di calcolo.

Il progetto fornisce uno strumento essenziale per lavoratori turnisti, studenti o freelance, permettendo di avere un riscontro immediato dei propri guadagni in tempo reale. Inoltre, la natura di **Progressive Web App (PWA)** consente l'installazione su dispositivi mobili e desktop, garantendo il funzionamento anche in assenza di connessione internet.

L'applicazione utilizza il `localStorage` del browser per la persistenza dei dati, assicurando la privacy dell'utente senza necessità di database esterni o account, e offre funzionalità di esportazione CSV per l'archiviazione mensile.

Gli obiettivi principali sono tre:

1.  Offrire una visione chiara e immediata dei turni settimanali e mensili tramite un'interfaccia grafica.
2.  Automatizzare il calcolo dello stipendio gestendo variabili complesse come straordinari percentuali e bonus domenicali.
3.  Garantire la portabilità e la persistenza dei dati in locale, massimizzando la privacy e la velocità d'uso.

Questo progetto è stato sviluppato per un uso personale specifico, potrebbe non essere adatto ad altri tipi di usi.


## Autori
Il progetto è stato sviluppato da:

- **Simon Carbone**

## Tecnologie

Il progetto è stato realizzato con l'aiuto di:

![HTML5](https://img.icons8.com/color/48/html-5--v1.png) **HTML5** - Linguaggio di markup utilizzato per la strutturazione semantica dei contenuti e l'accessibilità.  

![CSS3](https://img.icons8.com/color/48/css3.png) **CSS3** - Utilizzato per lo styling dell'interfaccia.

![JavaScript](https://img.icons8.com/color/48/javascript--v1.png) **JavaScript (Vanilla)** - Linguaggio di scripting utilizzato per tutta la logica di calcolo.


 ## Progetto
 
Non è necessario clonare il codice per utilizzare l'applicazione. **Calendario Lavorativo** è accessibile direttamente via browser ed è installabile come applicazione nativa.

📌 Accesso Diretto

- Visita il sito ufficiale del progetto:  
  👉 [https://zimuch.github.io/Calendario-Lavorativo/](https://zimuch.github.io/Calendario-Lavorativo/)

📲 Installazione App (PWA)

L'applicazione è una **Progressive Web App**. Questo significa che puoi installarla sul tuo dispositivo per usarla a schermo intero e offline, proprio come un'app scaricata dallo store.

- **Su Android (Chrome):** Premi il menu (⋮) in alto a destra e seleziona **"Installa app"** o **"Aggiungi a schermata Home"**.
- **Su iOS (Safari):** Premi il tasto Condividi e seleziona **"Aggiungi alla schermata Home"**.
- **Su PC/Mac (Chrome/Edge):** Clicca sull'icona di installazione (💻⬇️) che appare nella barra degli indirizzi.


▶️ Utilizzo
- Una volta aperta l'app, imposta la tua **Paga Oraria** nelle impostazioni in alto.
- Clicca su un giorno qualsiasi per aggiungere un turno.
- I tuoi dati verranno salvati automaticamente nel dispositivo.

## Introduction
![English](https://img.icons8.com/color/48/great-britain-circular.png) 

**Calendario Lavorativo** (Work Calendar) is a project designed to simplify shift management and automate salary calculations, offering a modern and responsive alternative to traditional spreadsheets.

The project provides an essential tool for shift workers, students, or freelancers, allowing immediate real-time feedback on their earnings. Furthermore, its nature as a **Progressive Web App (PWA)** allows installation on mobile and desktop devices, ensuring operation even without an internet connection.

The application uses the browser's `localStorage` for data persistence, ensuring user privacy without the need for external databases or accounts, and offers CSV export functionality for monthly archiving.

The objectives are threefold:

1.  To provide a clear and immediate view of weekly and monthly shifts through a visual interface.
2.  To automate salary calculation by managing complex variables such as percentage-based overtime and Sunday bonuses.
3.  To ensure local data portability and persistence, maximizing privacy and speed of use.

This project was developed for specific personal use, it may not be suitable for other types of uses.


## Authors

- **Simon Carbone**

## Tools

The tools used for accomplishing the project were:

![HTML5](https://img.icons8.com/color/48/html-5--v1.png) **HTML5** - Markup language used for semantic content structuring and accessibility.   

![CSS3](https://img.icons8.com/color/48/css3.png) **CSS3** - Used for interface styling.

![JavaScript](https://img.icons8.com/color/48/javascript--v1.png) **JavaScript (Vanilla)** - Scripting language used for all calculation logic.


 ## How to use it yourself
 
You don't need to clone the code to use the application. **Calendario Lavorativo** is directly accessible via browser and can be installed as a native-like application.

📌 Direct Access

- Visit the official project site:  
  👉 [https://zimuch.github.io/Calendario-Lavorativo/](https://zimuch.github.io/Calendario-Lavorativo/)

📲 App Installation (PWA)

The application is a **Progressive Web App**. This means you can install it on your device to use it full-screen and offline, just like an app downloaded from a store.

- **On Android (Chrome):** Tap the menu (⋮) in the top right and select **"Install app"** or **"Add to Home screen"**.
- **On iOS (Safari):** Tap the Share button and select **"Add to Home Screen"**.
- **On PC/Mac (Chrome/Edge):** Click the install icon (💻⬇️) that appears in the address bar.

▶️ Usage

- Once the app is open, set your **Hourly Rate** in the top settings.
- Click on any day to add a shift.
- Your data will be automatically saved on your device.


## Contributors
Al progetto hanno partecipato:

<a href="https://github.com/Zimuch/Calendario-Lavorativo/graphs/contributors">
<img src="https://contrib.rocks/image?repo=Zimuch/Calendario-Lavorativo" />
</a>
